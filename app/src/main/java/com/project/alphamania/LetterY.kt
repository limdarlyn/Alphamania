...
class LetterY : AppCompatActivity(), TextToSpeech.OnInitListener {

    private lateinit var tts: TextToSpeech
    private var yakSound: MediaPlayer? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        yakSound = MediaPlayer.create(this, R.raw.sound_y)

        val yakImage = findViewById<ImageView>(R.id.yakImage)
        val backButton = findViewById<Button>(R.id.backButton)
        val nextButton = findViewById<Button>(R.id.nextButton)

        yakImage.setOnClickListener {
            speak("Y for Yak!") {
                yakSound?.start()

                // Horizontal sway to simulate movement or grazing
                val swayAnimator = ObjectAnimator.ofFloat(
                    yakImage,
                    "translationX",
                    0f, -40f, 40f, -40f, 40f, 0f
                )
                swayAnimator.duration = 2500 // Slightly slower sway for a grounded feel
                swayAnimator.repeatCount = ObjectAnimator.INFINITE
                swayAnimator.interpolator = LinearInterpolator()

                // Vertical bobbing to simulate grazing or standing
                val bobbingAnimator = ObjectAnimator.ofFloat(
                    yakImage,
                    "translationY",
                    0f, -15f, 0f, 15f, 0f
                )
                bobbingAnimator.duration = 3500
