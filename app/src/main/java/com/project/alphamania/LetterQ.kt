...
class LetterQ : AppCompatActivity(), TextToSpeech.OnInitListener {

    private lateinit var tts: TextToSpeech
    private var quailSound: MediaPlayer? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.letter_q)
        supportActionBar?.hide()

        tts = TextToSpeech(this, this)
        quailSound = MediaPlayer.create(this, R.raw.sound_q)

        val quailImage = findViewById<ImageView>(R.id.quailImage)
        val backButton = findViewById<Button>(R.id.backButton)
        val nextButton = findViewById<Button>(R.id.nextButton)

        quailImage.setOnClickListener {
            speak("Q for Quail!") {
                quailSound?.start()
